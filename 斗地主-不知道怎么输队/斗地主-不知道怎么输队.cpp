...
float CardCombo::getValue2() const//控手牌权值优化
{
	...
	bool biggestD  = isLikelyBiggest(comboLevel, 4, 204);
	bool biggestST  = isLikelyBiggest(comboLevel, 5, 705); // Treat straight as needing 5 cards
	bool biggestST2 = isLikelyBiggest(comboLevel, 6, 806); // Treat double-straight
	bool biggestPL  = isLikelyBiggest(comboLevel, 6, 906); // Treat plane
	bool biggestBOOM = true;
	for(Level l=comboLevel+1;l<13;++l){
		if(othersCards[l]>=4) { biggestBOOM = false; break; }
	}
...
/* Improved combo penalty: different combo types have different "playability" scores.
 * Singletons are hardest to play (high penalty), bombs/rockets are good (negative penalty).
 * This replaces the original flat roundValWithDecay * count penalty. */
float getComboPenalty(const comboDiv& divSeq) {
	float penalty = 0;
	for (const CardCombo& combo : divSeq) {
		switch (combo.comboType) {
			case CardComboType::SINGLE:    penalty += 12; break;
			case CardComboType::PAIR:      penalty += 8;  break;
			case CardComboType::TRIPLET:   case CardComboType::TRIPLET1:
			case CardComboType::TRIPLET2:  penalty += 5;  break;
			...
			case CardComboType::PLANE2:    penalty += 4;  break;
			case CardComboType::BOMB:
			case CardComboType::ROCKET:    penalty += -5; break;
...
void CardCombo::ValueCompare(vector<Card>& divcards,vector<Card>& cuts, comboDiv& DivRemain, float& MaxValue){
	...
	if( CutCombo.comboType == CardComboType::TRIPLET or
		...
		float penalty1 = getComboPenalty(TempSeq.div) + 7; /* +7 for the CutCombo itself */
