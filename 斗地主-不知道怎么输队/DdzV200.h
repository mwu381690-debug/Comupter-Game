...
#define Max_level 17 //3-10对应的level为3-10 J-2对应为11-15 小王16 大王17 
...
struct Ddz
{
	...
	char sCommandOut[800];	//通信输出内容
	char sDebug[280];		//调试输出中间变量 
...
struct CardCombo;
typedef vector<CardCombo> comboDiv; //牌型序列，比如{"AAA4","33","5","6"} 
typedef struct ComboDiv{//注意！！这里是大写的首字母 
	comboDiv div;
	float Value;//附加价值 或 总价值，待定 
}COMBODIV;
CardCombo HelpPass(struct Ddz * pDdz);
int Maxlevel(int iCards[]);						//所有牌中的最大点数
...
//出炸弹,采取边拆边判断能不能大过上家的策略 
void Bomb(struct Ddz * pDdz){
	SortById(pDdz->iOnHand);
	int cnt=CountCards(pDdz->iOnHand); //计算牌的总张数 
	if (102==pDdz->iLastTypeCount || cnt<4){
		//手牌小于4张或需要压的牌是火箭则必定没有可行方案 
		return;
	}  
	int i;
	//通过牌编码的性质找炸弹
	if(204==pDdz->iLastTypeCount)
	{
		for (i=3;pDdz->iOnHand[i]>=0;i++){
			if(pDdz->iPlaCount+1<kPlaMax
					...
					&& card2level(pDdz->iOnHand[i]) > pDdz->iLastMainPoint) //大于上家的主点数 
			{ 
...
{   int i,j,k,iLength,iTemp,iCount,iBiaoji,iFlag;
	int iCopyCards[21];
	iCount=CountCards(pDdz->iOnHand);
	iLength=pDdz->iLastTypeCount-700; //顺子的长度 
	SortById(pDdz->iOnHand);
	for(i=0;i<21;i++) iCopyCards[i]=pDdz->iOnHand[i];
	for(i=0;iCopyCards[i]>=0;i++)
		if (card2level(iCopyCards[i])>MAX_STRAIGHT_LEVEL) iCopyCards[i]=-1; //2,大王小王不能连到顺子里 

	for(i=1;iCopyCards[i]>=0;i++)
		if(card2level(iCopyCards[i])==card2level(iCopyCards[i-1])) iCopyCards[i-1]=-1; //去除点数相同的牌 
...
void LinkDouble(struct Ddz * pDdz){
	...
	int iCard=0;
	SortById(pDdz->iOnHand);
	iLength=(pDdz->iLastTypeCount-800)/2; //双顺的长度，记录几连对
	 
	for(i=0;i<21;i++) iCopyCards[i]=pDdz->iOnHand[i];
	for(i=0;iCopyCards[i]>=0;i++)
		if(card2level(iCopyCards[i])>MAX_STRAIGHT_LEVEL) iCopyCards[i]=-1; //2和大王小王不能算作连牌 
	...
	//三连和对子变成单牌，对子在上一个操作中已经变成了单牌 
	
	//以上将数组iCopyCards[21]处理成能组成对子的单牌数组且最大为A
...
void LinkThree(struct Ddz * pDdz){
	if(CountCards(pDdz->iOnHand)<6) return; //手牌张数小于6，则不可能有飞机
	int iTempArr[15][5]; //iTempArr[i][j] 表示level为i的牌有j张
	memset(iTempArr,-1,sizeof(iTempArr)); 
	int i,j,k;
	int iLength = pDdz->iLastTypeCount-900; //三顺的长度 
	int iTemp;
	 
	SortById(pDdz->iOnHand);    //先将数组进行排序
	for(i = 0 ; pDdz->iOnHand[i] > -1 ; i++)
	{
		if(pDdz->iOnHand[i] < 48) //2和大王小王不能当做顺子 
		{
			j = 1;
...
void LinkThreeSingle(struct Ddz * pDdz)
{
	...
	for(i = 0 ; pDdz->iOnHand[i] > -1 ; i++)
	{
		if(pDdz->iOnHand[i] < 48)
		{
			...
			//将编码放入iTempArr[card2level(pDdz->iOnHand[i])][j]里面，因为该编码的点数为card2level(pDdz->iOnHand[i])
			iTempArr[card2level(pDdz->iOnHand[i])][j] = pDdz->iOnHand[i];

			//将点数为 card2level(pDdz->iOnHand[i]) 的牌的张数 改为 j ；
			iTempArr[card2level(pDdz->iOnHand[i])][0] = j;
		}
	}
	...
	for(i = pDdz->iLastMainPoint+1 ; i <15 ; i++)
	{
		...
		if(iTempArr[i][0]>=3) //相应点数的牌大于等于三张 
		{
			j = i;
			while (iTemp)
			{
			...
			if(iTemp == 0) //达到了目标长度，则生成出牌方案并寻找带牌 
			{
